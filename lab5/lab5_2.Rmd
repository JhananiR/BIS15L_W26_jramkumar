...
1. Load the data
```{r}
fishes <- read_csv("Gaeta_etal_CLC_data.csv")
```
```{r}
glimpse(fishes)
```
...
```{r}
filter(fishes, between(scalelength, 2, 4))
```
...
filter(penguins, species=="Chinstrap" | body_mass_g>4500)
```
Using or: | = meets either one of the conditions. 
Rules:  
