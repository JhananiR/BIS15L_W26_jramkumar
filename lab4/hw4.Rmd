...
ggplot(data=msleep, 
  ...
  labs(title="# of Mammals in Each Vore Type")
```
The herbivores are most represented in the data.

**4. What is the average length of time that mammal sleeps?**
```{r}
mean(msleep$sleep_total)
```
The average length of time that mammals sleep is about 10.4 hours. 
...
ggplot(data=msleep, 
  ...
  scale_y_log10()+
  labs(title="Range of Body Weight by Each Vore Type")
```
...
ggplot(data=msleep, 
  ...
  scale_y_log10()+
  labs(title="Total Sleep Time (hours) vs Body Weight (kg)")
```
There is somewhat of a negative relationship between total sleep time and body weight. As the number of total hours of sleep increases, the body weight decreases. 
...
ggplot(data=msleep, 
  ...
  geom_boxplot(mapping=aes(fill=vore))+
  scale_y_log10()+
  labs(title="# of Hours of Sleep in Each Vore Type")
```
Comparing the medians, the insectivores tend to sleep the most. 
...
```
There is a positive relationship between brain weight and body weight. 
...
```{r}
ggplot(data=msleep, 
       mapping=aes(x=sleep_total, y=brainwt))+
  geom_point(mapping=aes(color=vore))+
  geom_smooth(method=lm, se=T)+
  scale_y_log10()+
  labs(title="Total Sleep Time (hours) vs Brain Weight (kg)")
```
There is a negative correlation between the hours of sleep and brain weight. As the number of hours of sleep an animals receives increases, the brain weight decreases.
