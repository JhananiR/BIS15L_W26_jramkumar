...
```{r}
mammals %>% 
  mutate(across(everything(), tolower)) #this changes all the variables to characters, even the numeric variables
```
...
```{r}
mammals %>% 
  mutate(across(c(mass, wean_mass, gestation, max_life, newborn, weaning, litter_size, litters_year, afr),
         ~ifelse(.==-999, NA, .)))
```
...
Common name for the species with the highest mass is the blue whale.
...
positive correlation between gestation and newborn mass. 

4. Which mammal has the longest life span in years?  
```{r}
mammals %>% 
  select(genus, species, max_life) %>% 
  mutate(max_life_yrs=max_life/12) %>% 
  arrange(desc(max_life))
```

mammal with the longest life span in years is fin whale
