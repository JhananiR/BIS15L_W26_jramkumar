...
msleep %>% 
  ...
  summarize_all(mean) #mean of all numeric variables
```
...
msleep %>% 
  summarize(across(where(is.numeric), 
                   ~mean(.x, na.rm = T))) #tilda is function, .x = for every cell
```
...
msleep %>% 
  filter(bodywt > 150) %>% 
  distinct() #output of full dataframe
```

11 distinct observations
...
msleep %>% 
  filter(bodywt>100) %>% 
  summarize(total=n()) #not looking at unique number of genus
```
...
msleep %>% 
  filter(bodywt > 150) %>% 
  distinct(genus) %>% #now this only gives all the unique genera
  pull(genus) #list of the unique genera but not distinct ones
```
...
```{r}
msleep %>% 
  count(vore, sort = T) #sorts into descending order
```
...
msleep %>% 
  ...
  geom_col() #use this when we have an x and y
```
